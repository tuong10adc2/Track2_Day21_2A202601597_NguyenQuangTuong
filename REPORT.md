...
- **Cac buoc can tai khoan cloud/GitHub that** (tao bucket, service account, VM, GitHub Secrets, `git push`, cho GitHub Actions chay, `curl` toi VM那 that) khong the tu dong hoa tu moi truong lam viec cuc bo — nhung phan nay duoc ghi lai chi tiet trong checklist ban giao kem theo de nguoi dung tu thuc hien.
